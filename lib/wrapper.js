...
wrapper.configure = function (config) {
  ...
  _apiInstance = new circleauth.CircleAuthApi()
  _isInitialized = true
}
...
wrapper.getSession = async function (sessionID) {
  try {
    ...
    const generatedSignature = generateQueryStringSignature({ sessionID }, _writeKey)
    const returnValue = await _apiInstance.getSession(_appKey, sessionID, generatedSignature)
    checkJsonSignature(returnValue)
    return returnValue
  } catch (error) {
    console.log(error)
